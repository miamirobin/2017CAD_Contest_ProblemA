...
// prove PI symmetry
bool CirMgr::provePiSymmetry(SatSolver* solver,
  ...
  for (unsigned i = 0; i < nPi; ++i)
    if (i != idx0 && i != idx1)
      assump.push(mkLit(ctrl[i]));
  if (!inv) {
    assump.push( mkLit(piVar0[idx0]));
    assump.push(~mkLit(piVar0[idx1]));
    assump.push(~mkLit(piVar1[idx0]));
    assump.push( mkLit(piVar1[idx1]));
  } else {
    assump.push( mkLit(piVar0[idx0]));
    assump.push( mkLit(piVar0[idx1]));
    assump.push(~mkLit(piVar1[idx0]));
