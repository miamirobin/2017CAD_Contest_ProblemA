...
bool CirMgr::readparse(const std::string& filename) {
  std::string tmpFilename = filename + ".tmp";
 if (!removeComment(filename, tmpFilename)) return false;
   
  if (!parsePatch(tmpFilename)) return false;
   //cout<<7788;
...
bool CirMgr::readout(const std::string& filename) {
  std::string tmpFilename = filename + ".tmp";  
   if (!removeComment(filename, tmpFilename)) return false;
  if (!parseOut(tmpFilename)) return false;
    //cout<<8888;
...
bool CirMgr::parseOut(const std::string& filename) {
  std::ifstream ifs(filename.c_str());
  std::string str;
  std::vector<std::string> tokens;
 // init();
  ...
  while (!ifs.eof()) {
    str = readUntil(ifs, ';');
    tokens = split(str, " ,;()\t\r\n");
    unsigned n = tokens.size();               //  for (int i=0;i<n;i++){cout<<tokens[i]<<" ";}cout<<endl;
    if (n == 0) continue;
    ...
    if (tokens[0] == "module") {
      ...
      for (int i=0;i<(tokens.size()-2) /2;i++){
        ...
        CirGate* gate=getGateByName(tokens[i*2+2]);
        gate->setName(tokens[i*2+3]);  //cout<<"t:"<<tokens[i*2+3];
      }
    } else if (tokens[0] == "endmodule") {
      // endmodule
      break;
...
bool CirMgr::parsePatch(const std::string& filename) {
  std::ifstream ifs(filename.c_str());
  std::string str;
  std::vector<std::string> tokens;
  //init();
  ...
  while (!ifs.eof()) {         
    ...
    if (tokens[0] == "module") {
      ...
      assert(n >= 2);
      for (unsigned i = 1; i < n; ++i){
        std::string name="."+tokens[i];   //cout<<name;
        addPiGate(name);
      }
    } else if(tokens[0] == "output") {
      // output <name0> <name1> ...
      ...
      for (unsigned i = 1; i < n; ++i) {
        std::string name="."+tokens[i];  // cout<<name;
        CirGate* poGate = addPoGate(name);
        CirGate* aiGate = addAIGate(name);
        connectPo(poGate, aiGate);
      }
    } else if ((tokens[0] == "wire") and(tokens[1]!="t_0")) {
      // wire <name0> <name1> ...
      ...
      CirGate* out = name2GateMap_[name1];
      CirGate* in0 = name2GateMap_[name2];
      connect1inputGate(tokens[0], out, in0);
    } else if(tokens[0] == "and" || tokens[0] == "nand" ||
              tokens[0] == "or"  || tokens[0] == "nor"  ||
              tokens[0] == "xor" || tokens[0] == "xnor" ) {
      ...
      in0 = name2GateMap_[name1];
      in1 = name2GateMap_[name2];
      for (unsigned i = 4; i < n; ++i) {
        out = addAIGate("");
        connect2inputGate(type, out, in0, in1);
        in0 = out;
      ...
      out = name2GateMap_[name];                    //cout<<name<<" "<<name1<<" "<<name2<<endl;
      connect2inputGate(tokens[0], out, in0, in1);
    }
  }
  ifs.close();    
