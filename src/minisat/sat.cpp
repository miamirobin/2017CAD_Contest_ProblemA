...
const bool SatSolver::simplify() { return solver_->simplifyDB();}

void SatSolver::addUnitCNF(Var var, bool val) {
  solver_->addUnit(mkLit(var, !val));
}

void SatSolver::addEqCNF(Var var0, Var var1, bool inv) {
  Lit lit0 = mkLit(var0, false);
  Lit lit1 = mkLit(var1, inv);
  solver_->addBinary( lit0, ~lit1);
  solver_->addBinary(~lit0,  lit1);
}

void SatSolver::addxorCNF(Var var0, Var var1, bool inv) {
  Lit lit0 = mkLit(var0, false);
  Lit lit1 = mkLit(var1, inv);
  solver_->addBinary( lit0, lit1);
  solver_->addBinary(~lit0, ~lit1);
...
void SatSolver::addAndCNF(Var out, Var in0, bool inv0, Var in1, bool inv1) {
  ...
  // out = in0 & in1
  // clause (in0 + ~out)
  solver_->addBinary(in0Lit, ~outLit);
  // clause (in1 + ~out)
  solver_->addBinary(in1Lit, ~outLit);
  // clause (~in0 + ~in1 + out)
  solver_->addTernary(~in0Lit, ~in1Lit, outLit);
}
