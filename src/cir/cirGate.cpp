...
// report fanin2
void CirGate::reportpatch()const {
    
     reportpatch_rec(0, false);
    
}

// recursively report fanin
void CirGate::reportpatch_rec(unsigned indent, bool inv) const {
  for (unsigned i = 0; i < indent; ++i) std::cout << "  ";
  if (inv) std::cout << "!";
  report(false);
  if (getWeight()>-1){std::cout<<std::endl;return;}
  ...
  if (isFlg()) {
    ...
    if (!in0_.isNull()) {
      if (!in0_.isFloat()){
        in0_.gate()->reportpatch_rec(indent+1, in0_.isInv());
      }
      else
        std::cout << "Z" << std::endl;
    }
    if (!in1_.isNull()) {
      if(!in1_.isFloat()){
        in1_.gate()->reportpatch_rec(indent+1, in1_.isInv());
      }
      else
        std::cout << "Z" << std::endl;
    }
...
void CirAIGate::genCNF(SatSolver* solver) const {
  //std::cout<<name_<<" "<<in0_.gate()->getName()<<" "<< in1_.gate()->getName()<<std::endl;
