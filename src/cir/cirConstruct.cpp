...
CirGate* CirMgr::addWtGate(const string& name) {
  CirGate* tGate = new CirPiGate(name);
  gateList_.push_back(tGate);
  //piList_.push_back(tGate);
  tList_.push_back(tGate);
  if (name != "")addToMap(name, tGate);
  return tGate;
}
...
void CirMgr::connectnot(CirGate* out,  CirGate* in0, bool inv0) {
  out->setFanin(CirGateV(in0, inv0));
  in0->addFanout(CirGateV(out, inv0));
