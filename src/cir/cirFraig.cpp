...
void CirMgr::fraig(int effort) {
  ...
  for (unsigned i = 0, n = topoList_.size(); i < n; ++i) {
    ...
    if (gate->getEqGate().gate() != 0) {
     ...
     // if (gate->getName()=="G" or ""){replaceGate(gate, gate->getEqGate());}
      //else { if (gate->getEqGate().gate()->getName()!="" and gate->getEqGate().gate()->getWeight()<=gate->getWeight()){replaceGate(gate, gate->getEqGate());}}
...
void CirMgr::addCircuitCNF2(SatSolver* solver) { 
  CirGate::incFlg();                     
  for (unsigned i = 0, n = topoList_.size(); i < n; ++i) {
    
    CirGate* gate = topoList_[i];  //cout<<gate->getName()<<" ";
    gate->setToFlg();
    gate->setVar(solver->newVar());
    gate->genCNF(solver);
  }
  for (unsigned i = 0, n = aigList_.size(); i < n; ++i) {
     CirGate* gate = aigList_[i];  //cout<<gate->getName()<<" ";
